...
if __name__=='__main__':
    ...
    for j in range(1,max_iter):
        for k in range(1,max_iter):
            print(j,k)
            if os.path.exists('Traditional Momentum/Strategy/L/'+period+'/'+str(j)+'-'+str(k)+'.csv'):
                print('Already Exists')
                portfolio_value=pd.read_csv('Traditional Momentum/Strategy/L/'+period+'/'+str(j)+'-'+str(k)+'.csv')
                ...
                try:
                    ...
                    break
                portfolio_value.to_csv('Traditional Momentum/Strategy/L/'+period+'/'+str(j)+'-'+str(k)+'.csv')
