...
for j in range(1,8):
    for k in range(1,8):
        if k<=j:
            ...
            port_value.to_csv('Buying/Week/momentum_1/Turnover Rate/'+str(j)+str(k)+'.csv')
            port_value.plot()
            a=k*data['Close']
            a.plot()
            plt.show()
...
for j in range(,8):
    for k in range(1,8):
        if k<=j:
            ins,outs=momentum_3(matrix3,j=j,k=k,equal=False,contrarian=True)
            port_value=portfolio_long_value(ins,outs,8000,k=k)
            port_value.to_csv('Buying/Week/momentum_3/'+str(j)+str(k)+'.csv')
            port_value.plot()
            a=k*data['Close']
            a.plot()
            plt.show()